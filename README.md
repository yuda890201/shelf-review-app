...
コメントモーダル・タグ編集シート・ピン投稿の入力欄は、いずれもブラウザ標準の `<dialog>` 要素(`showModal()`)で実装しており、背景のタップ・フォーカスはブラウザ自身に無効化させています。それでもiOS Safariでは背景のタッチスクロールまでは止まらないことがあったため、実績のある `body-scroll-lock` パッケージ(`src/lib/use-body-scroll-lock.ts`)を併用し、モーダル内の該当スクロール領域だけは通常通りスクロールできるようにしつつ、背景のスクロールを止めています。
