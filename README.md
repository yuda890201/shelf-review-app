...
| `/` | Instagram風の縦スクロールフィード(投稿ごとに「いいね」「まだまだ」ボタンと反応率、新規作成ボタン) |
| `/sessions/new` | 店舗選択→売場選択→カメラ起動のウィザード形式でセッション作成 |
...
`sessions`(1枚の画像を囲むブレスト会) → `images`(売場画像、Storageパスと店舗/棚カテゴリのメタ情報) → `comments`(相対座標0〜1のピン位置 + good/bad種別 + 本文 + 投稿者)。`reactions` はセッション単位で1ユーザー1件(`unique(session_id, user_id)`)の「いいね/まだまだ」を保持し、フィードの反応率計算に使う。
...
- コメントの投稿者は `author_id`(user id)として構造化データに保存されますが、コメント一覧のUI上は「自分」/「参加者」の区別のみ表示しています。
