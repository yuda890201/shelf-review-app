...
| `/layouts/[id]` | 本部お手本写真(春/秋・年を指定してアップロード、最新のものが表示される)と、選択した店舗の現在の売場写真を左右に並べて比較。どちらの写真もタップして流れるコメントピンを貼り付けられる(`pins`テーブル、`src/components/photo-annotator.tsx`)。写真の下にはタスク(コメント)入力欄もあり、チェックで完了/未完了を切り替えられる(店舗ごとに独立したタスクリスト) |
