...
| `/` | Instagram風の縦スクロールフィード。サムネイル写真の上にも投稿済みの流れるコメントピン(`src/components/pin-chip.tsx`)がそのまま表示され、スクロール中も常にアニメーションし続ける。投稿ごとに「🙏ありがとう」(何回でも送れる応援)と「✅完成/🔧まだまだ」(1人1票、完成率を可視化)のボタン、写真の連続タップでも🙏を送信、💬アイコンでページ遷移なしにピン打ち・コメントができるモーダルを開く、📤アイコンで投稿リンクを共有 |
| `/sessions/new` | 店舗選択→売場選択→カメラ起動のウィザード形式でセッション作成。カメラロールからは最大5枚まで一括選択でき、1枚ごとに個別のセッションとして作成される |
| `/sessions/[id]` | セッション詳細 — 画像上にピンを打って良い点/悪い点コメントを投稿、他の参加者の投稿がリアルタイムで反映される。「よく使う文言」タグをタップ即投稿できるスタンプも表示し、タグ横の「✎ タグを編集」から手動での追加・編集・削除もできる(`tag-manager-modal.tsx`)。司会者(セッション作成者)はクローズ操作が可能。ピン打ちの中核ロジックは `sessions/[id]/pin-board.tsx` に切り出してあり、フィードのモーダルとこのページの両方から使われる。ピンの見た目(流れるコメントの枠)自体は `src/components/pin-chip.tsx` として共通化されており、フィードのサムネイル表示とも共有している |
