...
コメントモーダル・タグ編集シート・ピン投稿の入力欄は、いずれもブラウザ標準の `<dialog>` 要素(`showModal()`)で実装しており、背景のタップ・フォーカスはブラウザ自身に無効化させています。それでもiOS Safariでは背景のタッチスクロールまでは止まらないことがあったため、実績のある `body-scroll-lock` パッケージ(`src/lib/use-body-scroll-lock.ts`)を併用し、モーダル内の該当スクロール領域だけは通常通りスクロールできるようにしつつ、背景のスクロールを止めています。いずれの`<dialog>`も`createPortal`で`document.body`直下に描画しており、ネストした親要素(スクロール領域など)によるクリッピングを受けません。

ピン投稿の入力欄(コメント種別・タグスタンプ・本文・角度調整)は、画面下部からせり上がる形ではなく画面上部から下りてくる形式にしています。これはスマホの画面下部からせり上がるソフトウェアキーボードと競合しない位置にするための設計です。
