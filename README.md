...
コメントモーダル・タグ編集シート・ピン投稿の入力欄は、いずれもブラウザ標準の `<dialog>` 要素(`showModal()`)で実装しています。独自のCSS/JSで背景スクロールを止める実装は、iOS Safariでは背景がスクロールしてしまう不具合が解消できなかったため廃止し、背景の無効化(スクロール・タップとも)をブラウザ自身に保証させる形にしています。
