...
export default async function CommentsPage({
  ...
  return (
    <div>
      <h1 className="mb-4 text-lg font-bold">コメント一覧</h1>
        ...
        {comments?.map((c) => (
          <li key={c.id}>
            <Link
              ...
              className="block rounded-md border border-gray-200 bg-white p-3 hover:border-blue-300"
            >
              <div className="mb-1 flex items-center gap-2 text-xs">
                ...
                {c.images?.store_name && (
                  <span className="text-gray-400">/ {c.images.store_name}</span>
                )}
                {c.images?.shelf_category && (
                  <span className="text-gray-400">/ {c.images.shelf_category}</span>
                )}
